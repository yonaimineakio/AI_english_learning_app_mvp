...
def upgrade() -> None:
    ...
    op.add_column(
        "review_items", sa.Column("new_user_id", sa.String(36), nullable=True)
    )
    op.add_column(
        "saved_phrases", sa.Column("new_user_id", sa.String(36), nullable=True)
    )
...
def _drop_all_fk_constraints_for_column(
...
def _upgrade_mysql(conn) -> None:
    ...
    conn.execute(
        text("ALTER TABLE sessions CHANGE new_user_id user_id VARCHAR(36) NOT NULL")
    )

    conn.execute(text("ALTER TABLE review_items DROP COLUMN user_id"))
    conn.execute(
        text("ALTER TABLE review_items CHANGE new_user_id user_id VARCHAR(36) NOT NULL")
    )

    conn.execute(text("ALTER TABLE saved_phrases DROP COLUMN user_id"))
    conn.execute(
        text(
            "ALTER TABLE saved_phrases CHANGE new_user_id user_id VARCHAR(36) NOT NULL"
        )
    )
