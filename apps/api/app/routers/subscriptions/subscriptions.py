...
async def process_revenuecat_webhook(request: Request, db: Session = Depends(get_db)):
    ...
    if event_type in {"INITIAL_PURCHASE", "RENEWAL", "UNCANCELLATION"}:
        ...
        return JSONResponse(status_code=status.HTTP_400_BAD_REQUEST, content={"message": "Unknown event type"})

    logger.info(f"RevenueCat webhook completed: {request}")
    return JSONResponse(status_code=status.HTTP_200_OK, content={"message": "Webhook processed successfully"})